...
void medicion(void) {
  ...
  Serial.print(String(cont) + "Distancia: ");
  Serial.print(distancia);
  Serial.print(" cm");
  Serial.println("");
  valores[cont] = distancia;
  delay(100);
  cont++;
...
void dibujar(void) {
  for (int y = 0; y < datos; y++) {
    ...
    graf3 = map(valores_salida1[y], 0, 1200, 0, 255);
    //graf2 = map(1000 * valores_gauss[y] / suma, 0, 1200, 0, 255);
    graf1 = map(valores_salida[y], 0, 1200, 0, 255);
    gra = map(valores[y], 0, 1200, 0, 255);
